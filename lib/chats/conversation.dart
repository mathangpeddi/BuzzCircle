...
import 'package:fp2/models/enum/message_type.dart';
import 'package:fp2/components/chat_bubble.dart';
import 'package:fp2/models/message.dart';
import 'package:fp2/models/user.dart';
import 'package:fp2/pages/profile.dart';
import 'package:fp2/utils/firebase.dart';
import 'package:fp2/view_models/conversation/conversation_view_model.dart';
import 'package:fp2/view_models/user/user_view_model.dart';
...
class _ConversationState extends State<Conversation> {
  ...
  if (msg.isNotEmpty) {
    Message message = Message(
      content: msg,
      senderUid: user?.uid,
      messageId: chatRef.doc(widget.chatId).collection('messages').doc().id,
      type: isImage ? MessageType.IMAGE : MessageType.TEXT,
      time: Timestamp.now(),
    );
  print("Sending message from user: ${user?.uid}");
    if (isFirst) {
      String id = await viewModel.sendFirstMessage(widget.userId, message);
      print("--------------First-----------");
      print("First message sent with chatId: $id");
      setState(() {
        isFirst = false;
        chatId = id;
        chatIdRef.add({
        ...
        chatRef.doc(id).update({'reads': {}, 'typing': {}});
      });
    } else {
